...
#define DEFAULT_COUNTDOWN_DURATION 500
#define TMPDEFAULT_COUNTDOWN_DURATION 0
...
unsigned int tmpcountdownSeconds = 500;
...
void loop() {
  ...
  if (buttonPressedNew(DET_BUTTON)) {
    ...
    detPressed = true;
    //countdownSeconds = defaultCountdownSeconds;
    countdownSeconds = tmpcountdownSeconds;
    displayCountdown = true;
  }
  ...
  if (!buttonPressed(DET_BUTTON)) {
    if (detPressed) {
      ...
      Serial.println(tmpcountdownSeconds);
      detPressed = false;
      if(digitalRead(WIRE_1)==LOW && digitalRead(WIRE_2)==LOW && digitalRead(WIRE_3)==LOW && digitalRead(WIRE_4)==LOW){
        defaultCountdownSeconds = tmpcountdownSeconds;
        writeEEPROM();
        etape = 0;
        countdown(); 
      }
...
void cutError() {
  tmpcountdownSeconds = countdownSeconds;
  countdownSeconds = 0;
}
...
void countdown() {
  ...
  // assign random pins
  int defuse_pins[4] = {WIRE_1,WIRE_2,WIRE_3,WIRE_4};
  defusePin =  WIRE_1; //random(WIRE_1, (WIRE_4+1));
  detPin = WIRE_2; //defusePin;
  ...
  while ((countdownSeconds > 0) && (!defused)) {
    for(int i=0;i<10000;i++) {
      ...
      for(int fil=etape; fil< 4; fil++){
        if(digitalRead(defuse_pins[fil]) == HIGH){
           if((fil==0 or fil==1) and countdownSeconds%2==0)
           {
            if(fil == etape){
              etape++;
              if(etape > 3){
           ...
           else if((fil==2 or fil==3) and countdownSeconds%2==1)
           {
            if(fil == etape){
              etape++;
              if(etape > 3){
